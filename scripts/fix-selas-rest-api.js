  ...
  for (const collectionName of COLLECTIONS_TO_CHECK) {
    ...
    try {
      ...
      for (const doc of documents) {
        ...
        const selasId = (data.selasId && data.selasId.stringValue) || data.selasId || '';
        
        if (!selasId || selasId === '') {
          try {
            // Identifier l'élément pour le log
            let elementName = 'N/A';
            if ((data.nom && data.nom.stringValue)) elementName = data.nom.stringValue;
            else if ((data.name && data.name.stringValue)) elementName = data.name.stringValue;
            else if ((data.registrationNumber && data.registrationNumber.stringValue)) elementName = data.registrationNumber.stringValue;
            else if ((data.immatriculation && data.immatriculation.stringValue)) elementName = data.immatriculation.stringValue;
